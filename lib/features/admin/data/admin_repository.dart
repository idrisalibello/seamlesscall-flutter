  ...
  Future<List<Map<String, dynamic>>> getProviderApplications() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception(
          'Failed to load provider applications. Status code: ${response.statusCode}',
        );
      }
      ...
      if (e.response != null) {
        ...
        throw Exception(
          'Failed to load provider applications: ${e.response?.data['messages'] ?? e.message}',
        );
      } else {
  ...
  Future<void> approveOrRejectProvider(int userId, String action) async {
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        print(
          'Failed to $action provider application. Status code: ${response.statusCode}',
        );
        throw Exception(
          'Failed to $action provider application. Status code: ${response.statusCode}',
        );
      ...
      if (e.response != null) {
        ...
        print('Error response status: ${e.response?.statusCode}');
        throw Exception(
          'Failed to $action provider application: ${e.response?.data['messages'] ?? e.message}',
        );
      } else {
        print('Error sending request: ${e.message}');
        throw Exception('Error sending request: ${e.message}');
  ...
  Future<void> createAdminUser({
    ...
    try {
      ...
      if (response.statusCode == 201) {
        ...
        throw Exception(
          'Failed to create admin user. Status code: ${response.statusCode}',
        );
      }
    } on DioException catch (e) {
      if (e.response != null) {
        ...
        throw Exception(
          'Failed to create admin user: ${e.response?.data['messages'] ?? e.message}',
        );
      } else {
        print('Error sending request: ${e.message}');
        throw Exception('Error sending request: ${e.message}');
  ...
  Future<List<Category>> getCategories() async {
    try {
      ...
      if (e.response?.statusCode == 404) {
        throw Exception(
          'The requested categories were not found (404). Please check the API endpoint.',
        );
      }
      throw Exception(
    ...
    try {
      ...
      if (response.statusCode == 200) {
        try {
          ...
          print(
            'Error parsing category details. Raw response: ${response.data}',
          );
